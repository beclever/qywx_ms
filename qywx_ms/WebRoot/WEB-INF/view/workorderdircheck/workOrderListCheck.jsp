						...
						<c:forEach items="${result.pendingWorkform }" var="data" varStatus="num">
							<div class="gdlb_list">
								...
								<span class="gdlb_info"> 工 单 号：${data.poNumber }<br />设备序列号：${data.serialNumber}<br />
									...
									<br /> 状态：主任审核
									<br />
								</span> <a href ="${ctx_path }/cp/workOrder/dealWorkOrder.do?poNumber=${data.poNumber }&workformId=${data.workformId }&equipmentId=${data.equipmentId }&detailType=2"><span class="query_arrow"></span></a>
							</div>
					 	</c:forEach>
					</div>
