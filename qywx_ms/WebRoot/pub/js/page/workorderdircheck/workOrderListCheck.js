...
function getPageData(pageNum){
	...
	$.ajax( {  
        type : "post",  
        url : basePath+"/cp/workOrderDirCheck/pageList.do",
				...
				if(dataJson.pendingWorkform!=null&&dataJson.pendingWorkform.length>0){
					...
					$(dataJson.pendingWorkform).each(function(i,data){
						...
						"<span class=\"gdlb_info\"> 工 单 号："+data.poNumber+"<br />"+ "设备序列号："+ data.serialNumber+"<br />" +
							...
							"<br /> 状态：主任审核"+
							"<br />"+
						"</span> <a href =\""+basePath+"/cp/workOrder/dealWorkOrder.do?poNumber="+data.poNumber+"&workformId="+data.workformId+"&equipmentId="+formatData(data.equipmentId)+"&detailType=2"+"\"><span class=\"query_arrow\"></span></a>"+
					"</div>";
						$('#thelist').append(html);
					});
				}else{
