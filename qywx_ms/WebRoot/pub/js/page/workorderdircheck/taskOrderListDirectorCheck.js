//2013.10.25
	...
	function orderPass(){
		...
		var timeReportTime=$("#timeReportTime").val();
		...
		if(leaveTime >= timeArriveTime){
			layer.alert("到达时间应大于出发时间 ！",{closeBtn: false});
			return;
		}else if(timeReportTime >= leaveTime){
			layer.alert("出发时间应大于报修时间 ！",{closeBtn: false});
			return;
		}else if(timeArriveTime >= timeStartWorkTime){
			layer.alert("到达时间应小于开始时间！",{closeBtn: false});
			return;
		}else if(timeStartWorkTime >= timeFinishWorkTime){
			layer.alert(" 开始时间应小于完成时间！",{closeBtn: false});
			return;
		}else if(timeReportTime >= timeArriveTime){
			layer.alert(" 到达时间应大于报修时间！",{closeBtn: false});
			return;
		}else if(timeReportTime >= timeAppointmentDate){
			layer.alert("预约时间应大于报修时间 ！",{closeBtn: false});
			return;
		}
		...
		layer.load(2,{ shade: [0.6,'#666666'] });//加载中...
		  $.ajax({
		        ...
		        data:{
		        	poNumber:poNumber,
		        	reportTime:timeReportTime,
		        	appointmentDate:timeAppointmentDate,
	...
	function orderReturnResult(){
		document.getElementById("select_content_new").style.display="none";
		var url = basePath+"/cp/workOrderDirCheck/orderReturn.do?poNumber="+poNumber+"&equipmentId="+equipmentId+"&workformId="+workformId;
		window.location.href=url;
	}
	// 时间设置
...
// 巡检
    ...
    $("#chargeadd").click(function () {
        ...
        var html = "<div class='work_task_inputcontent'>" +
            ...
            +"</div>" +
            // "<span class='rright'><input type='image'
			// src='css/sa/images/delete.png' onclick='WS(this)'/></span>" +
			// //去掉删除按钮
    ...
    $("#historyProblemadd").click(function () {
        ...
        if(historyproblemremark ==''){
        	...
        	var html = "<div class='work_task_inputcontent'>" +
	            ...
	            "</li>" +
	            "</div>" +
	            // "<span class='work_task_inputcontent_img rright'><input
				// type='image' src='css/sa/images/delete.png'
				// onclick='WS(this)'/></span>" + //去掉删除按钮
...
function loadData(obj, oldSerialName, oldSerialNumber, oldMaterialCode, oldHardwareVersion, oldSoftwareVersion, equipmentConfigId, newSerialName, newMaterialCode, newSerialNumber, newHardwareVersion, newSoftwareVersion) {
    var htmlstr = "<div class='task_box02'><span><font color='blue'>原物料:</font><input type='hidden' value='" + oldSerialName + "' name='" + obj + "oldSerialName'/>" +
        ...
        + newSerialName
        + "</span><input type='image' class='logistics_close' src='"+basePath+"images/delete.png' onclick='WSSpare(this)'/></div>";
    $("#" + obj).append(htmlstr);
}
